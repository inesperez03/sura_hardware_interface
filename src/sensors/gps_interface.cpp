...
bool GpsInterface::initialize(const hardware_interface::HardwareInfo & info)
{
  if (initialized_) {
    return true;
  }

  if (const auto it = info.hardware_parameters.find("gps_protocol");
    it != info.hardware_parameters.end())
  {
    protocol_ = it->second;
  }

  if (const auto it = info.hardware_parameters.find("gps_serial_port");
    it != info.hardware_parameters.end())
  {
    serial_port_ = it->second;
  }

  if (const auto it = info.hardware_parameters.find("gps_baudrate");
    it != info.hardware_parameters.end())
  {
    try {
      ...
      RCLCPP_WARN(
        kLogger,
        "Invalid gps_baudrate '%s': %s. Using default %d",
        it->second.c_str(),
        ex.what(),
        baudrate_);
    }
  ...
  RCLCPP_INFO(
    kLogger,
    "Configured GPS interface: protocol='%s', serial_port='%s', baudrate=%d",
    protocol_.c_str(),
    serial_port_.c_str(),
    baudrate_);
  ...
  double & valid)
{
  ...
  latitude = last_latitude_;
  longitude = last_longitude_;
